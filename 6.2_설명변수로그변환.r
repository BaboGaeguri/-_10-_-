...
model1 <- lm(Endemics_ratio ~ log_Area + Elevation + Nearest + Scruz + log_Adjacent, data = df)
summary(model1)
...
model1 <- lm(Endemics_ratio ~ log_Area + Elevation + Nearest + Scruz + log_Adjacent, data = df)
summary(model1)
model2 <- lm(Endemics_ratio ~ log_Area, data = df)
summary(model2)

# =============================
# 6. model2로 기본가정 다시 진단
# =============================
# 독립성, 등분산성(잔차 그림, DW-test)
y_hat_model1 <- fitted(model1)
resid_model1 <- residuals(model1)
...
# =============================
# 7. 설명 변수 간의 다중공선성과 변수 선택
# =============================
# a. 다중공선성 파악
# 1) 설명변수들 간의 상관행렬에서 절댓값이 큰 상관계수를 갖는 경우
library(corrplot)
X <- df[, c("log_Area", "Elevation", "Nearest", "Scruz", "log_Adjacent")]
cor_mat <- cor(X)
...
# b. 변수 선택
full_model <- lm(Endemics_ratio ~ log_Area + Elevation + Nearest + Scruz + log_Adjacent, data = df)
...
# 4) 모든 가능한 회귀
library(leaps)
all_subsets <- regsubsets(Endemics_ratio ~ log_Area + Elevation + Nearest + Scruz + log_Adjacent,
                          data = df,
...
# =============================
# 8. 변수 축소전 설명변수와 반응변수 구성요소간 상관관계 파악
# =============================
# 상관계수 검정 (Endemics와 각 설명변수)
cor.test(df$Endemics, df$log_Area)
cor.test(df$Endemics, df$Elevation)
cor.test(df$Endemics, df$Nearest)
cor.test(df$Endemics, df$Scruz) 
cor.test(df$Endemics, df$log_Adjacent)

# 상관계수 검정 (Species와 각 설명변수)
cor.test(df$Species, df$log_Area)
cor.test(df$Species, df$Elevation)
cor.test(df$Species, df$Nearest)
cor.test(df$Species, df$Scruz)
cor.test(df$Species, df$log_Adjacent)
...
# Nearest와 Scruz 묶어버리기
model1 <- lm(Endemics_ratio ~ log_Area + Elevation + Nearest + Scruz + log_Adjacent, data = df)
summary(model1)
model2 <- lm(Endemics_ratio ~ log_Area + I(Nearest + Scruz) + log_Adjacent, data = df)
summary(model2)
anova(model2, model1) # F 검정

# Area와 Elevation 묶어버리기
model1 <- lm(Endemics_ratio ~ log_Area + Elevation + Nearest + Scruz + log_Adjacent, data = df)
summary(model1)
model2 <- lm(Endemics_ratio ~ I(Elevation / Area) + Nearest + Scruz + log_Adjacent, data = df)
summary(model2)
anova(model2, model1) # F 검정

model1 <- lm(Endemics_ratio ~ log_Area + Elevation + Nearest + Scruz + log_Adjacent, data = df)
summary(model1)
model2 <- lm(Endemics_ratio ~ I(Area + Elevation), data = df)
summary(model2)
anova(model2, model1) # F 검정
...
model1 <- lm(Endemics_ratio ~ log_Area + Elevation + Nearest + Scruz + log_Adjacent, data = df)
summary(model1)
model2 <- lm(Endemics_ratio ~ log_Area + Scruz, data = df)
summary(model2)
anova(model2, model1) # F 검정

# 고립도지수화(Nearest/Adjacent)
model1 <- lm(Endemics_ratio ~ log_Area + Elevation + Nearest + Scruz + log_Adjacent, data = df)
summary(model1)
model2 <- lm(Endemics_ratio ~ log_Area + Elevation + Scruz + I(Nearest / Adjacent), data = df)
summary(model2)
anova(model2, model1) # F 검정

# Nearest, Adjacent inverse 변환
new_df <- df[, ]
new_df$inv_Nearest <- 1/(new_df$Nearest+1)
new_df$inv_Adjacent <- 1/(new_df$Adjacent+1)
model1 <- lm(Endemics_ratio ~ log_Area + Elevation + Nearest + Scruz + log_Adjacent, data = new_df)
summary(model1)
model2 <- lm(Endemics_ratio ~ log_Area + Elevation + inv_Nearest + Scruz + inv_Adjacent, data = new_df)
summary(model2)
anova(model2, model1) # F 검정

# =============================
# 10. log_Area만 있는 모델과 log_Area+Scruz 모델 진단 및 비교
# =============================
### model1 ###
model1 <- lm(Endemics_ratio ~ log_Area, data = df)
summary(model1)
...
### 잔차 플롯 ###
# a. 등분산성 확인(0을 중심으로 랜덤하게 분포하고 패턴이 없어야함.)
y_hat_model2 <- hatvalues(model2)
student_resid <- rstudent(model2)

par(mfrow = c(1, 1)) # 예측값(ŷ)과 잔차(ε̂) 산점도
plot(y_hat_model2, student_resid,
     pch = 19, col = "blue",
     xlab = expression(hat(Y)), 
     ylab = expression(ri),
     main = "예측값(ŷ)과 스튜던트화 잔차 산점도")
